...
export default function ProfileCard({ imageSrc, title, description }) {
  ...
  // animate both in and out on repeated visits
  const inView = useInView(ref, { amount: 0.35 }); // no "once"

  // Split title to accent the word "Human"
  const parts = useMemo(() => title.split(/(Human)/gi), [title]);

  // Parent controls layout + stagger
  const parent = {
    hidden: {
      opacity: 0,
      y: 32,
      ...
      transition: {
        ...
        staggerDirection: -1, // reverse on the way out
      },
    },
    show: {
      opacity: 1,
